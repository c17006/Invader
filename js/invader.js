...
phina.define('MainScene', {
    ...
    init: function () {
        ...
        for (let i = 0; i < 4; i++) {
            for (let j = 0; j < 12; j++) {
                Enemy(this.gridX.span(10 + j * 2), this.gridY.span(5 + i * 4), ENEMY_ASSETS[Math.floor(Math.random() * ENEMY_ASSETS.length)]).addChildTo(this.enemyGroup);
                enemycount++;
            }
        }
...
phina.define('Enemy', {
    ...
    update: function (app) {
        ...
        if (this.missiletime >= this.missileinterval) {
            this.missiletime = 0;
            let randomNum = Math.floor(Math.random() * 10)
            if (randomNum === 0) {
                Missile(this.x, this.bottom).addChildTo(this.parent.parent.missileGroup);
            }
        }
    },
